...
for path in all_versions:
    ...
    try:
        ...
        print(
            'Skipped saved file with invalid version "{}": {}'.format(raw_version, path)
        )
    except Exception:
        continue
...
def prompt_user_for_version(sorted_candidates):
    ...
    try:
        ...
        tde4.addLabelWidget(
            requester, REQUESTER_LABEL_WIDGET, label, "ALIGN_LABEL_LEFT"
        )
        ...
        button = tde4.postCustomRequester(
            requester, window_title, 420, 300, "Load", "Cancel"
        )
...
def resolve_version_to_load(candidates):
    ...
    if chosen is None:
        ...
        print(
            "No version picked, loading the latest saved version (v{:04d}).".format(
                latest_version
            )
        )
