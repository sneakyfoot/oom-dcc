...
def _next_version(sg, ctx, pf_type_id: int, name_code: str) -> int:
    ...
    versions = sorted({p.get('version_number') for p in pubs if p.get('version_number') is not None})
...
def _latest_publish_version(sg, ctx, pf_type_id: int, name_code: str) -> int:
    ...
    versions = [int(p.get('version_number')) for p in pubs if p.get('version_number') is not None]
...
def _latest_version_entity_version(sg, ctx, name_code: str) -> int:
    ...
    vers = sg.find('Version', filters, ['code'], order=[{'field_name': 'id', 'direction': 'desc'}])
...
def _apply_template_dir(tk, context, template_name: str, name: str, version: int, frame_pad: int = 4) -> tuple[str, str]:
...
def _render_write(write_node, first: int, last: int, incr: int = 1):
    ...
    try:
        ...
        if hasattr(nukescripts, 'executeInBackground'):
            try:
                ...
                nukescripts.executeInBackground('OOM Render', [write_node], int(first), int(last), int(incr))
            except Exception:
                nukescripts.executeInBackground('OOM Render', [write_node.fullName()], int(first), int(last), int(incr))
            return
    ...
    try:
        if hasattr(nuke, 'executeInBackground'):
            nuke.executeInBackground('OOM Render', [write_node.fullName()], int(first), int(last), int(incr))
            return
    except Exception:
        pass
...
def _create_publish(sg, ctx, pf_type_id: int, name: str, sg_seq_path: str, version: int) -> dict:
...
def _encode_mp4_with_ffmpeg(seq_printf_path: str, start: int, fps: float, out_path: str, crf: int = 18) -> None:
    import re
    ...
    try:
        ...
        if not os.path.exists(first_frame_path):
            raise RuntimeError(f"Sequence not found: expected first frame '{first_frame_path}'")
...
def _upload_version(sg, ctx, task, code: str, movie_path: str, start: int, end: int, fps: float, description: str | None = None) -> dict:
...
def render_and_publish_exr(gizmo_node, name: str | None = None, pft_code: str = 'oom_comp', version: int | None = None, write_node_name: str = 'write_exr'):
    ...
    return {'publish': pub, 'sequence': sg_seq, 'version': version, 'first': first, 'last': last}


def render_and_upload_mp4(gizmo_node, name: str | None = None, version: int | None = None, fps: float | None = None, crf: int = 18, pft_code: str = 'oom_comp', write_node_name: str | None = 'write_mp4'):
    ...
    if version is None:
        ...
        candidates = [v for v in (ver_from_exr, latest_pub_v, latest_disk_v) if int(v) > 0]
    ...
    if write_node_name and w:
        try:
            ...
            raise RuntimeError(f"MP4 write via internal node '{write_node_name}' failed: {e}")
    else:
        # No dedicated node found — use ffmpeg as a fallback only in this case
        _encode_mp4_with_ffmpeg(sg_seq, start=first, fps=fps, out_path=mp4_path, crf=crf)
    ...
    return {'version_entity': ver, 'movie_path': mp4_path, 'first': first, 'last': last, 'fps': fps}
