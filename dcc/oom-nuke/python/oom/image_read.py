...
class ReadVersionsDialog(QtWidgets.QDialog):
    ...
    def _create(self):
        ...
        if not path:
            QtWidgets.QMessageBox.critical(self, 'Invalid Publish', 'Missing local_path.')
            return
...
class InsertReadDialog(QtWidgets.QDialog):
    ...
    def _refresh(self):
        ...
        try:
            ...
            fields = ['code', 'name', 'version_number', 'path']
            pubs = self.sg.find('PublishedFile', filters, fields,
...
def update_selected_read_to_latest():
    ...
    pubs = sg.find('PublishedFile', [
        ['project', 'is', proj],
        ['entity', 'is', ent],
        ['code', 'is', key],
    ], ['code', 'version_number', 'path'], order=[{'field_name': 'version_number', 'direction': 'desc'}])
